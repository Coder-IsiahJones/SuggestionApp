...
public class MongoSuggestionData : ISuggestionData
{
   ...
   public async Task<List<SuggestionModel>> GetAllSuggestionsWaitingForApproval()
   {
      ...
      return output
         .Where(x => x.ApprovedForRelease == false && x.Rejected == false)
         .ToList();
   }
   ...
   public async Task UpvoteSuggestion(string suggestionId, string userId)
   {
      ...
      try
      {
         ...
         var suggestionsInTransactions = db.GetCollection<SuggestionModel>(_db.StatusCollectionName);
         ...
         await usersInTransactions.ReplaceOneAsync(u => u.Id == userId, user);
